...
class WebhooksObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        switch ($eventName)
        {
            ...
            case 'stripe_payments_webhook_charge_succeeded':
                ...
                $order = $this->webhooksHelper->loadOrderFromEvent($arrEvent);
                $order = $this->reloadUnfinalized($order);
                ...
                if ($shouldInvoice)
                {
                    if ($amountCaptured == 0 && !$hasSubscriptions) // If it has trial subscriptions, we want a Paid invoice which will partially refund
                    {
                ...
                else if ($amountCaptured > 0)
                {
                    ...
                    if ($invoiceCollection->count() > 0)
                    {
                        ...
                        $this->paymentsHelper->sendInvoiceEmail($invoice);
                    }
                }
    ...
    public function reloadUnfinalized($order)
    {
        // On slow development/staging sites, charge.succeeded arrives too fast. The observer runs and finishes before
        // the original order placement is fully committed to the database. We give Magento a few seconds to finalize the order
        // before performing any post authorization actions.
        ...
        do
        {
            if ($info->getAdditionalInformation("finalized"))
                return $order;

            sleep($step);
            $wait -= $step;
            $order = $this->paymentsHelper->loadOrderByIncrementId($order->getIncrementId(), false);
            $info = $order->getPayment()->getMethodInstance()->getInfoInstance();
        }
        while ($wait > 0);

        // We finalize it here in case its a payment method that does not redirect back to the site.
        $info->setAdditionalInformation("finalized", true);
        $info->save();
